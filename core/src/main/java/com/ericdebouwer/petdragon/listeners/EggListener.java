...
public class EggListener implements Listener {
	...
	public void eggPlace(BlockPlaceEvent e){
		...
		//New version of egg place: event ALWAYS is canceled, check is delayed, egg gets removed from hand an given back if no spawn is possible!
		...
		Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, () -> {
			e.getBlock().setType(Material.DRAGON_EGG,false);	//place an egg (fake the event result)		//may be removed again.. block is protectet by interact event...for 0.5 seconds..
			e.getBlock().setMetadata("isPetEgg", new FixedMetadataValue(plugin,true));//Set a marker for the 'dontTouch'-event :)
		});
		
        UUID uuid = e.getPlayer().getUniqueId();
        plugin.getDragonLocations().getLocationList(uuid); //returns a list with valid Locations for this player and activates the chunks! (list only if the chunks have been activated already!)
        Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, () -> {
        	if (plugin.getConfigManager().isEggAbidesDragonMax() && !e.getPlayer().hasPermission("petdragon.bypass.dragonlimit")) {
//     		if (plugin.getConfigManager().useMaxDragonsForEggs && !e.getPlayer().hasPermission("petdragon.bypass.dragonlimit")){	//Jeppa's version until 1.5.3
...
//				e.getItemInHand().setAmount(1);
			} else { //Hand has Item, but not a PetEgg, drop the item, so the player can pick it up...
	...
	public void dontTouchTheEgg(PlayerInteractEvent event){
		if (event.getClickedBlock().hasMetadata("isPetEgg")){ //Block has temporary meta -> is placed Block for hatching...
