...
public class PetEnderDragon_v1_15_R1 extends EntityEnderDragon  implements PetEnderDragon {
		...
		try {
			if (!IRegistry.ENTITY_TYPE.getOptional(mcKey).isPresent()) {
				...
				IRegistry.a(IRegistry.ENTITY_TYPE, PetEnderDragon.ENTITY_ID,
						EntityTypes.a.a(PetEnderDragon_v1_15_R1::new, EnumCreatureType.MONSTER).a().a(PetEnderDragon.ENTITY_ID));
			}
			...
			checkCrystals = EntityEnderDragon.class.getDeclaredMethod("es");
	...
	public PetEnderDragon_v1_15_R1(org.bukkit.World world) {
	...
	public void movementTick(){
		...
		if (this.world.isClientSide) {
			...
			if (!this.isSilent()) { //noises
				float f = MathHelper.cos(this.by * 6.2831855F);
				float f1 = MathHelper.cos(this.bx * 6.2831855F);
				if (f1 <= -0.3F && f >= -0.3F) {
		...
		if (this.getHealth() <= 0.0F) { //dead
			...
			if (!this.world.isClientSide && this.hurtTicks == 0) {
				...
				this.hurt(this.world.getEntities(this, this.bw.getBoundingBox().g(1.0D), IEntitySelector.e));
			...
			if (!this.world.isClientSide && plugin.getConfigManager().isDoGriefing()) { //more efficient grieving check
				try {
					checkWalls.invoke(this, this.bw.getBoundingBox());
	...
	private void knockBack(List<Entity> list) {
		...
		for (Entity entity : list) {
			if (entity instanceof EntityLiving) {
				...
				if (didMove && ((EntityLiving) entity).cI() < entity.ticksLived - 2) {
